...
public class ExposicionPerros
{
    ...
    public void ordenarPorRaza( )
    {
    	...
    	for (int i = perros.size(); i > 0; i--) {
    		for (int j = 0; j < i-1; j++) {
    			Perro perroActual = (Perro) perros.get(j+1);
    			Perro perroSiguiente = (Perro) perros.get(j);
				if (perroActual.darRaza().compareTo(perroSiguiente.darRaza()) < 0) {
					Perro temp = perroActual;
					perros.set(j+1, perroSiguiente);
					perros.set(j, temp);
    ...
    public void ordenarPorNombre( )
    {
    	...
    	if(perros != null) {
    		for (int i = 1; i < perros.size(); i++) {
    			for (int j = i; j > 0; j--) {
    				Perro perroActual = (Perro) darPerros().get(j);
        			Perro perroSiguiente = (Perro) darPerros().get(j-1);
        			
        			if (perroActual.darNombre().compareTo(perroSiguiente.darNombre()) < 0) {
        				Perro temp = perroActual;
        				perros.set(j, perroSiguiente);
    					perros.set(j-1, temp);
    				}
    ...
    public void ordenarPorPuntos( )
    {
    	...
    	for (int i = perros.size(); i > 0; i--) {
    		for (int j = 0; j < i-1; j++) {
    			Perro perroActual = (Perro) perros.get(j+1);
    			Perro perroSiguiente = (Perro) perros.get(j);
				if (perroActual.darPuntos() < perroSiguiente.darPuntos()) {
					Perro temp = perroActual;
					perros.set(j+1, perroSiguiente);
					perros.set(j, temp);
    ...
    public void ordenarPorEdad( )
    {
    	...
    	for (int i = 0; i < perros.size(); i++) {
    		Perro menorPerro = (Perro) perros.get(i);
    		int cualPerro = i;
    		for (int j = i+1; j < perros.size(); j++) {
    			Perro actualPerro = (Perro) perros.get(j);    			
				if (actualPerro.darEdad() < menorPerro.darEdad()) {
					menorPerro = (Perro) perros.get(j);
					cualPerro = j;
				}
			}
    		Perro temp = (Perro) perros.get(i);
    		perros.set(i, menorPerro);
    		perros.set(cualPerro, temp);
		}
